...
https://opensource.org/licenses/BSD-3-Clause[image:https://img.shields.io/badge/License-BSD%203--Clause-blue.svg[License]]

This is a simple variation on the well known
http://codingdojo.org/kata/FizzBuzz/ kata exercise.

=== The interview exercise

According to the instructions, the exercise have been developed in an
iterative way, so the branch `main` contains only the specification
`Step 1`.

The most part of the exercise is in the branch `alfresco` that contains
all the remaining steps.

The installation instruction will make reference to the more complete
branch `alfresco`.
...
NOTE: Using the GraalVM (https://www.graalvm.org/), it will be possible
to generate native versions of the FizzBuzz applicaton NOTE: The docker
and docker compose version indicated are the ones used to develop and
test the application. It is expected that the application works also
...
package the docker image using docker compose:
...
By default, the Java project will package the application in a
`fast-jar` format, that allows a quicker startup time for the docker
container.
...
and launched as a stand alone server.
...
At the end of the build process, there eventually are two jars and one
docker image and a native executable application available to the user:
...
The application take the number of fizzbuzz word to generate as an input
parameter. For instance, to obtain 10 fizzbuzz items:
...
Executing the following command from the terminal to start up the
FizzBuzz server:
...
to launch the docker container. Again, to access the web application
